...
func (s *ProxyServer) ListenES(){
	...
	log.Printf("Wiseplat Stratum listening on %s", s.config.Proxy.Stratum.Listen)
...
func(cs *Session) getNotificationResponse(s *ProxyServer, id *json.RawMessage) JSONRpcResp {
	...
	param1[2] = "WiseplatStratum/1.0.0"
	result[0] = param1
	result[1] = cs.Extranonce

	resp := JSONRpcResp{
		Id:id,
		Version:"WiseplatStratum/1.0.0",
		Result:result,
		Error: nil,
	}
...
func(cs *Session) sendESReq(resp WshStratumReq)  error {
	cs.Mutex.Lock()
	defer cs.Mutex.Unlock()
...
func (cs *Session) handleESMessage(s *ProxyServer, req *StratumReq) error {
	...
	case "mining.subscribe":
		...
		if params[1] != "WiseplatStratum/1.0.0"{
			log.Println("Unsupported stratum version from ", cs.ip)
			return cs.sendESError(req.Id, "unsupported wiseplat version")
		}
	...
	case "mining.authorize":
		...
		respReq := WshStratumReq{Method:"mining.set_difficulty", Params:paramsDiff}
...
func (s *ProxyServer) broadcastNewESJobs() {
	...
	log.Printf("Broadcasting new job %s to %v wiseplat stratum miners", job.JobID, count)
